...
static RTC_DATA_ATTR uint32_t packet_count = 0;

uint32_t ttn_rejoin_period = DEFAULT_TTN_REJOIN_PERIOD;
...
if(packet_count == 0) {
    ...
    if(p.begin("lora", true)) {
        packet_count = p.getUInt("count", 0);
        p.end();
    }
}
}
...
bool rejoin_ttn(uint8_t downlinkArray[], uint8_t len){
    /*
    Called when we receieve a TTN downlink starting with "RR" (1: R for a reset setting, 2: R for rejoin).
    If the downlink is of a valid length (6), and ends with our RESET_CONFIRM_STRING,
    then we reset our TTN perfs and stop reseting our WDT timer to trigger a system restart
    ...
    */
    ...
    if(len == 6 && downlinkString.substring(2,len).equals(RESET_CONFIRM_STRING)) {
        Serial.println("TTN Rejoin Request Starting");
        ttn_send(downlinkArray, len, LORAWAN_PORT, false);
        ttn_erase_prefs();
        return false;
    }
    else {
        Serial.print("TTN Rejoin Request Authentication Failed: len was "); Serial.print(len); 
        Serial.print(" confirm_string was "); Serial.println(downlinkString.substring(2,len));
        return true;
    }
}

void set_ttn_rejoin_period(uint8_t downlinkArray[], uint8_t len) {
    /*
    Called when we receieve a TTN downlink starting with "IR" (1: I for set interval, 2: R for rejoin).
    ...
    */
    ...
    if(downlinkString.length()>=10 && downlinkString.substring(len-4,len).equals(RESET_CONFIRM_STRING)){
        Serial.print("setting rejoin period to: "); Serial.println((uint32_t)downlinkString.substring(2,len-4).toInt());
        ttn_send(downlinkArray, len, LORAWAN_PORT, false);
        ttn_rejoin_period = (uint32_t)downlinkString.substring(2,len-4).toInt();
        EEPROM.writeULong(18, (uint32_t)downlinkString.substring(2,len-4).toInt());
        EEPROM.commit();
    }else{
        Serial.print("set_ttn_rejoin_period received string was too short: "); Serial.println(downlinkString);
    }
}
